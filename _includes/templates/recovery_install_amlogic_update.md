...
1. Download `aml_install_package.img` file from [here](https://download.lineage.rgbroms.top/devices/{{ device.codename }}).
...
```
    {% include alerts/note.html content="These flash packages were built by LineageOS Revived maintainers from officially supported LineageOS Revived repositories, by using `lunch` to select the device, then run `m aml_install`." %}
...
5. The script will flash the firmware, and after you see `Resetting board [OK]`, plug in a USB keyboard and cycle power to the board, at which point you will see LineageOS Revived Recovery boot. You may now proceed with the following installation instructions.
...
7. Download `super_empty.img` from [here](https://download.lineage.rgbroms.top/devices/{{ device.codename }}).
