...
2. Download [Lineage Recovery](https://download.lineage.rgbroms.top/devices/{{ custom_recovery_codename }}). Simply download the latest recovery file, named `{{ device.recovery_partition_name }}.img`
...
{%- else %}
{%- assign is_lineage_recovery = true %}
2. [Build]({{ device | device_link: "build" | relative_url }}) a LineageOS Revived installation package. The recovery will be built as part of it!
