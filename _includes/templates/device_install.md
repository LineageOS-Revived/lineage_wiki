...
The provided instructions are for LineageOS Revived {{ current_branch }}. These will only work if you follow every section and step precisely. <br/>
...
{%- unless device.quirks contains "ims" %}
    {% include alerts/warning.html content="Make sure that you can send and receive SMS and place and receive calls (also via WiFi and LTE, if available), otherwise it won't work on LineageOS Revived either! Additionally, some devices require that VoLTE/VoWiFi be utilized once on stock to provision IMS." %}
{%- endunless %}
{%- endif %}
{%- unless device.stock_is_not_android %}
6. Remove all Google accounts from your device to avoid "Factory reset protection"
{%- endunless %}
7. LineageOS Revived is provided as-is with no warranty. While we attempt to verify [everything works](https://github.com/LineageOS-Revived/charter/blob/main/device-support-requirements.md) you are installing this at your own risk!
...
- LineageOS Revived builds for this device require an **Android {{ device.before_install.version }}** version of the [stock OS]({{ "glossary/#stock-rom" | absolute_url }}){: .glossary} to be installed prior to following the installation guide
- Please ensure that you are checking the **Android** version, and not the vendor OS version
{%- endif %}
- Being on another custom ROM, including unofficial builds of the same version of LineageOS Revived, does not ensure that this requirement has been fulfilled
...
Failing to install the correct firmware version prior to installation may result in failure to install LineageOS Revived, unexpected crashes post-installation,
...
## Installing LineageOS Revived from recovery
...
{%- else %}
1. [Build]({{ device | device_link: "build" | relative_url }}) a LineageOS Revived installation package.
{%- endif %}
...
{%- endif %}
6. Sideload the LineageOS Revived `.zip` package:
...
5. Copy the the LineageOS Revived `.zip` package and any addons you wish to install to your device's removable SD card, then insert it in your device and install it, but **do not reboot to system** before you read/followed the rest of the instructions!:
    * On the device, select "Apply update", then "Apply from EXT_SDCARD", and select the LineageOS Revived zip file to begin installation.
    {% include alerts/warning.html content="EXT_SDCARD will reflect the name of your SD card's filesystem name." %}
{%- else %}
5. Sideload the LineageOS Revived `.zip` package but **do not reboot to system** before you read/followed the rest of the instructions!
...
This add-on needs to be installed **before** booting into LineageOS Revived for the first time!
{%- endcapture %}
...
When presented with a screen that says `Signature verification failed`, click `Yes`. It is expected as add-ons aren't signed with LineageOS Revived's official key!
{%- endif %}
...
If you are wondering about the next steps, continue, otherwise, enjoy your new LineageOS Revived installation.
...
- LineageOS Revived is Android. It supports apps the same way as any other Android device, but apps might choose to exclude you from using them. Please go back to your [device's quirks section]({{ device | device_link | append: "#known-quirks" | relative_url }}) and read about "Device integrity".
