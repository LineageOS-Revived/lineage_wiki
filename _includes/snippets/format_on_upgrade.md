...
9. Download [Lineage Recovery](https://download.lineage.rgbroms.top/devices/{{ custom_recovery_codename }}). Simply download the latest recovery file, named `{{ device.recovery_partition_name }}.img`.
...
8. If your device is running LineageOS Revived version older than {{ device.format_on_upgrade | precision: 1 }}, wipe your data partition {{ factory_reset_naming }}.
{%- else %}
