...
Search and select `Generic Linux 2022` on the `Select the operation system you are installing` field. Then, click `Forward`.

#### Step 3

Specify the number of CPU cores and the size of memory that you're willing to allocate to the virtual machine.
The minimum RAM requirement is 2048 MiB. After populating these fields, click `Forward`.
...
6. Click `Finish`.
7. On the `Boot Options` tab, toggle `SATA CDROM 1` or `USB Disk 1`, and then click `Apply`.
...
{% include alerts/warning.html content="There is no guarantee that every GPU model will work fine without any problem. Initial testing used an Intel® HD Graphics 4600. If you have fixes for any GPU, feel free to submit!" %}
